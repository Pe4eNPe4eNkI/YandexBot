...
class DiscordBot(discord.Client):
    ...
    def get_currency_price_translate(self, number_1, alpha, beta):
        if self.alpha != "rub" and self.beta == "rub":  # Если одна из выбраных валют - рубль
            # Нахождение актуальной информации о валюте
            currency_now = float(get_currency_price(self.alpha)[0])
            number = float(self.number_1.replace(",", "."))
            answer = int(((currency_now * number)) * 1000) / 1000  # Оформление ответа
            return answer  # Запись ответа

        if self.alpha == "rub" and self.beta != "rub":  # Если одна из выбраных валют - рубль
            currency_now = float(get_currency_price(self.beta)[0])
            number = float(self.number_1.replace(",", "."))
            answer = int(((number / currency_now)) * 1000) / 1000  # Оформление ответа
            return answer  # Запись ответа

        elif self.alpha != "rub" and self.beta != "rub":  # Если ни одна из валют рубль
            # Нахождение актуальной информации о валюте
            currency_now_left = float(get_currency_price(self.alpha)[0])
            # Нахождение актуальной информации о валюте
            currency_now_right = float(get_currency_price(self.beta)[0])
            number = float(self.number_1.replace(",", "."))
